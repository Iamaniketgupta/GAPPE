...
export const chatSockets = (io) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("acceptCall", ({ roomId, user, accepterPeerId, callerPeerId }) => {
            socket.join(roomId);
            console.log(`User ${user.fullName} accepted the call and joined room: ${roomId}`);
        ...
        socket.on("declineCall", ({ roomId, user, targetId }) => {
            console.log(`User ${user.fullName} declined the call for room: ${roomId}`);
            activeCalls.delete(user._id);

            io.to(roomId).emit("callDeclined", { targetId: targetId, message: "Call was declined" });
        });
